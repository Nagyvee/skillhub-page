...
export async function generateStaticParams() {
    return business.courses.map((course) => ({
        slug: course.slug,
    }))
}

export default async function BusinessApplyPage({ params }: PageProps) {
    const { slug } = await params
    const course = business.courses.find((c) => c.slug === slug)
            ...
            <div className="relative pt-28 pb-20 z-10 flex-grow">
                ...
                <div className="relative z-10 max-w-3xl mx-auto px-6 lg:px-8 w-full">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mb-10">
                        ...
                        <span>/</span>
                        <Link href="/learnerships/business" className="hover:text-foreground transition-colors">Business</Link>
                        <span>/</span>
                        <Link href={`/learnerships/business/${slug}`} className="hover:text-foreground transition-colors truncate max-w-[120px]">{course.title}</Link>
                        <span>/</span>
