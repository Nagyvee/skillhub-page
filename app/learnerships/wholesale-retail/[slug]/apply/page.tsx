...
export async function generateStaticParams() {
    return wholesaleRetail.courses.map((course) => ({
        slug: course.slug,
    }))
}

export default async function WholesaleRetailApplyPage({ params }: PageProps) {
    const { slug } = await params
    const course = wholesaleRetail.courses.find((c) => c.slug === slug)
            ...
            <div className="relative pt-28 pb-20 z-10 flex-grow">
                ...
                <div className="relative z-10 max-w-3xl mx-auto px-6 lg:px-8 w-full">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mb-10">
                        ...
                        <span>/</span>
                        <Link href="/learnerships/wholesale-retail" className="hover:text-foreground transition-colors">Wholesale & Retail</Link>
                        <span>/</span>
                        <Link href={`/learnerships/wholesale-retail/${slug}`} className="hover:text-foreground transition-colors truncate max-w-[120px]">{course.title}</Link>
                        <span>/</span>
